...
/**
 * @brief Post a message to both Core 0 and Core 1 (using the Core 0 and Core 1 low-priority queues).
 *        Do not wait if it can't be posted.
 * @ingroup multicore
 *
 * Generally used for informational status. Especially information that
 * is updated on an ongoing basis. This uses the Low-Priority queue.
 *
 ...
 */
extern uint16_t post_to_cores_nowait(const cmt_msg_t* msg);
