...
#define _SM_OVERHEAD_US_PER_MS 20 // From testing (@SEE os._handle_be_test)
...
bool _schd_msg_timer_callback(repeating_timer_t* rt) {
    for (int i = 0; i < SCHEDULED_MESSAGES_MAX; i++) {
        ...
        if (smd->remaining > 0) {
            if (0 == --smd->remaining) {
                if (0 == smd->corenum) {
                    post_to_core0_blocking(smd->client_msg, MSG_PRI_NORMAL);
                }
                else {
                    post_to_core1_blocking(smd->client_msg, MSG_PRI_NORMAL);
                }
