...
typedef enum _MSG_PRIORITY_ {
    MSG_PRI_NORMAL = 0,
    MSG_PRI_HIGH_FALLBACK = 1,
    MSG_PRI_HIGH = 2,
    MSG_PRI_CRITICAL = 3
} msg_priority_t;

typedef enum _MSG_ID_ {
    // Common messages (used by both OS and Functional-Level)
    ...
    MSG_UI_INITIALIZED,
    //
    // Front-End/Functional-Level messages
    MSG_FL_NOOP = 0x0200,
    MSG_OS_INITIALIZED,
    MSG_DISPLAY_MESSAGE,
} msg_id_t;
...
#define postOSMsgBlocking( pmsg, pri )      post_to_core0_blocking( pmsg, pri )
#define postOSMsgIDBlocking( id, pri )      post_by_id_to_core0_blocking( id, pri )
#define postOSMsgNoWait( pmsg )             post_to_core0_nowait( pmsg )
#define postOSMsgIDNoWait( id )             post_by_id_to_core0_nowait( id )
#define postFLMsgBlocking( pmsg, pri )      post_to_core1_blocking( pmsg, pri )
#define postFLMsgIDBlocking( id, pri )      post_by_id_to_core1_blocking( id, pri )
#define postFLMsgNoWait( pmsg )             post_to_core1_nowait( pmsg )
#define postFLMsgIDNoWait( id )             post_by_id_to_core1_nowait( id )
#define postBothMsgBlocking( pmsg, pri )    post_to_cores_blocking( pmsg, pri )
#define postBothMsgIDBlocking( id, pri )    post_by_id_to_cores_blocking( id, pri )
#define postBothMsgNoWait( pmsg )           post_to_cores_nowait( pmsg )
#define postBothMsgIDNoWait( id )           post_by_id_to_cores_nowait( id )
